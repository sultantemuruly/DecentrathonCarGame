...
public class CarAI : MonoBehaviour
{
	...
	public void CheckObstacles()
	{
		...
		else if (Physics.Raycast(PosLeft, DirCenter, out hit, raycastLength * .5f, myLayerMask))
		{                   // Left
			Obstacles = "Left";
			ObstacleDistance = hit.distance;
		}
		else if (Physics.Raycast(PosRight, DirCenter, out hit, raycastLength * .5f, myLayerMask))
		{               // Right
			Obstacles = "Right";
			ObstacleDistance = hit.distance;
		}
		...
		if (Physics.Raycast(PosRight, -DirAngleRight, out hit, raycastLength * .7f, myLayerMask))
		{       // Right angle
			Obstacles = "Right";
			ObstacleDistance = hit.distance;
		...
		if (Physics.Raycast(PosLeft, -DirAngleLeft, out hit, raycastLength * .7f, myLayerMask))
		{           // Left angle
		...
		if (Physics.Raycast(obj_pivotCheckCarCollisionFrontRight.transform.position, CarDir01, out hit, 3 * .15f) && hit.collider.tag == "Car"
		   && Physics.Raycast(obj_pivotCheckCarCollisionRearRight.transform.position, -CarDir02, out hit, 3 * .15f) && hit.collider.tag == "Car")
		{                   // Check raycast with gameObject "CheckOtherCarCollision"
	...
	public void TurnLeftAndRight()
	{
		...
		if (!b_contactWithOtherCar)
		{
			...
			if (!b_DisableRotation)
			{
				if (Obstacles == "Null")
				{
					if (angle2 == 1 && Mathf.Abs(angle) > 2)
					{                                           // Check if the car need to turn left or right
			...
			{                                                                                   // Car go backward because she can't move
				if (Obstacles == "Right")
				{
					carController.Btn_RightActivate();
					carController.Btn_LeftDeactivate();
	...
	public void F_RandomCarValues()
	{
		if (allowRandomValues)
		{
			...
			if (successRate_BestTargetPos < randomValue)
			{                                                   // if random superior to success rate : Random Target local Position
			...
			if (successRate_BestOffsetSpeed < randomValue)
			{                                               // if random superior to success rate : Random Target local Position
			...
			if (successRate_BestOffsetRandomEulerAngle < randomValue)
			{                                   // if random superior to siccess rate : Random euler angle rotation
	...
	IEnumerator Take_Decision_Because_Car_Touch_Other_Car_On_His_Right()
	{
		...
		if (allowRandomValues)
		{                                                                               // add offset on targets that follow the cars
